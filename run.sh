...
(cd maelstrom && ./maelstrom test -w broadcast --bin ../broadcast/bin/broadcast --node-count 5 --time-limit 20 --rate 10 --nemesis partition)

go build -o bin/gocounter -C gocounter
(cd maelstrom && ./maelstrom test -w g-counter --bin ../gocounter/bin/gocounter --node-count 3 --rate 100 --time-limit 20 --nemesis partition)

go build -o bin/kafka -C kafka
(cd maelstrom && ./maelstrom test -w kafka --bin ../kafka/bin/kafka --node-count 2 --concurrency 2n --time-limit 20 --rate 1000)
